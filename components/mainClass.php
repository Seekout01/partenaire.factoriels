<?php
  ...
  public function validerForm($index=1, $inputArray=Null)
  {
    ...
    return $formValidated;
       ...
       try{
           $query = $this->connect->createCommand("SELECT * FROM $table WHERE code=:code")
                ...
                public function getAlltableData($table)
                {
                 ...
                 $query = null;
               try{
                   $query = $this->connect->createCommand("SELECT * FROM $table  WHERE codeentite= :codeentite")
                ...
                public function uniLibelle($table,$libelle,$columns){
                  ...
                  try{
                      $query = $this->connect->createCommand("SELECT * FROM $table where $columns = :libelle  AND codeEntite = :codeEntite")
                                ->bindValues([':libelle'=>$libelle,':codeEntite'=>$codeEntite])
                                ->queryAll();
                                if($query != null) return true;
                                return false;
                      }catch(\PDOException $ex){
                ...
                public function getAlltableFournisseursData($table)
                {
                 ...
                 $query = null;
               try{
                   $query = $this->connect->createCommand("SELECT * FROM $table  WHERE codeFournisseur=:codeEntite")
                                          ->bindValue(':codeEntite',$codeEntite)
                                          ->queryAll();
                             if($query != null) return $query;
                             return;
      ...
      public function afficherTitreFormulaire()
      {
       ...
       $libFiliere=Yii::$app->etsConfigClass->searchfiliere($codeFIlieere);
        ...
        $breadcrumb = '
                       ...
                       <a href="'.Yii::$app->request->baseUrl.'/'.md5("site_index").'" class="text-muted text-hover-primary">'.Yii::t('app','accueil').'</a>&nbsp;
                      </li>
                        ...
                        if(isset($headerFiliere)){
                          ...
                          <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
                              ...
                              <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
                                ...
                                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">'.Yii::t("app",$activeController).'</h1>
      ...
      {
        $rslt = $this->connect->createCommand('SELECT * FROM at_lien_user_ent WHERE codeUserAuth=:codeUserAuth and parDefault=:parDefault')
                              ->bindValues([':codeUserAuth'=>$codeUser, ':parDefault'=>1])
                              ->queryOne();
        if($rslt) return $rslt['codeentite'];
        return;
      }
      ...
      public function payslabel($pays)
      {
        ...
        if(isset($pays))
        {
          ...
          ->bindValue(':code',$pays)
          ->queryOne();
          if(sizeof($rslt)>0)$rslt=$rslt['nom'];
        }
        return $rslt;
      }
      ...
      public function getusertype($user)
      {
        if(isset($user))
        {
          ...
          ->bindValue(':id',$user)
          ->queryOne();
          if(isset($usertype) && sizeof($usertype)>0)$usertype = $usertype['typeUser'];
        }
        return $usertype;
      }
      ...
      Public function activeEntiveUserEnFnct($tbleauEntiteUserEnFnct){
        ...
        }else $activeEntiveUserEnFnct = Null;
      ...
      public function creerEvent($eventCode, $eventDesc){
        ...
        if(in_array($typeUser, Yii::$app->params['hostoUsr']))
        { ### DETERMINER SI CORPS MEDICAL

          if(is_array($arrayCorpMedHostos = mainCLass::ListCorpMedHostos($userAuthDtlsArray['idUniqUser'])))
          { ### DETERMINER LIST HOPITOS CORPS MEDICAL
            $actifHosto = mainCLass::actifHopitoParmiHostos($arrayCorpMedHostos);
          }else $actifHosto = Null;
        ...
        $rslt = $connect
                ->createCommand()->insert('ste_eventdetails', ['actorIdEntite'=>$idEntite,
        ...
        public function createnewsanteyakah($request, $nis, $menu, $realpass, $cryptedPass, $dteLimitAccess,$actor=0){
          ...
          if(is_array($rslt1)){
            ...
            ->bindValues([
                      ...
                      ':idUniq'=>$santeyahkaIdUniq, ':NIS'=>$nis, ':nom'=>$data['nom'], ':prenom'=>$data['pnom'], ':genre'=>$data['genre'], 
                      
                      ':tel'=>$data['cnt'], ':cntCasUrgence'=>$data['cnt_urgence'], ':email'=>$data['sonmail'], 
                      
                      ':dteNaiss'=>$datenaissance, ':lieuNaiss'=>$data['lieunaissance'], 
                      
                      ':profession'=>$profession, 
                      
                      ':stmatrimoniale'=>$data['stmatrimoniale'], ':nbre_enfant'=>$data['nbreenfant'],
                      
                      ':dteEnregitrement'=>date('Y-m-d'), 
                      
                      ':pays_residence'=>$data['paysresidence'], ':region_residence'=>$data['regionresidence'], ':prefecture_residence'=>$data['prefectureresidence'], ':adresse'=>$data['adresse'],

                      ':niveaudinstruction'=>$data['niveauscolaire'], ':lateralite'=>$data['lateralite'], ':statut'=>'1'
              ])
  ...
  $response = $client->request(
      ...
      [
          RequestOptions::MULTIPART => [
              ...
              ['name' => 'html', 'contents' =>  $this->render('/views/InitierEspace_dara/mail.html',['url'=>$content])],
    ...
    $query = null;
      try{
         ...
         // $this->connect->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $query = $this->connect->createCommand('INSERT INTO at_userauth(code,identifiant,motpass,modifiermotpass,typeuser,menuaction,statut,userActif) 
...
{
  $rslt = $this->connect->createCommand('INSERT INTO  at_barcode_tag(tag, numero) VALUES(:tag, :numero)')
...
{
  $rslt = $this->connect->createCommand('SELECT tag, numero FROM at_barcode_tag ORDER BY id DESC LIMIT 1')
                        ->queryOne();
  if($rslt) return $rslt;
  $rslt['tag'] = 'AA'; $rslt['numero'] = '0'; 
  return $rslt;
}
...
{
    $NumericNumber = intVal($number); $NumericNumber++;
    if( $number == "9999"){$NumericNumber=1;}
