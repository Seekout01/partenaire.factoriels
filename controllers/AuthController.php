...
class AuthController extends Controller {
	...
	public static function store_auth($code=Null, $identifiant=Null ,$typeUser=Null){
		...
		// $codeAnneeActive = $codeEts = Null;
		$codeEnt = yii::$app->mainCLass->chargerEntActive($code);
		
		$userAuthDtlsArray = ['userCode'=>$code,  'codeEnt'=>$codeEnt];
	...
	public static function UserAuthDtls($identifiant,$motPass){
		...
		if(!empty($identifiant)){
			...
			if(!is_array($auth) OR $auth==false){ # MEANING ,NO RECORD MATCHES THE userName SERCHED
				...
				$veraciteMot_passe = Yii::$app->cryptoClass->validate_password($motPass_constitue, $auth['motpass']); 
				...
				switch ($veraciteMot_passe) {
					...
					case 1: 
						...
						$storageProcess = AuthController::store_auth($auth['code'],$auth['identifiant'], $auth['typeuser']);
